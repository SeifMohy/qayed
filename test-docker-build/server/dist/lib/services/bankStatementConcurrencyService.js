 ...
 */
function areDateRangesExact(start1, end1, start2, end2) {
 ...
 */
export async function checkBankStatementConcurrency(accountNumber, bankName, statementPeriodStart, statementPeriodEnd, companyId) {
    ...
    for (const existingStatement of existingStatements) {
        ...
        if (existingBankName === newBankName) {
            ...
            // Check date ranges
            if (areDateRangesExact(statementPeriodStart, statementPeriodEnd, existingStatement.statementPeriodStart, existingStatement.statementPeriodEnd)) {
            ...
            else if (doDateRangesOverlap(statementPeriodStart, statementPeriodEnd, existingStatement.statementPeriodStart, existingStatement.statementPeriodEnd)) {
 ...
 */
export async function processBankStatementWithConcurrency(statement, supabaseUserId, fileName, fileUrl, rawTextContent) {
    ...
    const concurrencyCheck = await checkBankStatementConcurrency(statement.account_number, statement.bank_name, statementPeriodStart, statementPeriodEnd, companyId);
    ...
    switch (concurrencyCheck.action) {
        ...
        case 'MERGE_DIFFERENT_PERIOD':
            return await mergeTransactionsIntoExistingStatement(concurrencyCheck.existingStatementId, statement.transactions, statementPeriodStart, statementPeriodEnd, concurrencyCheck.reason);
        case 'ADD_TO_EXISTING_BANK':
            return await createNewStatementInExistingBank(concurrencyCheck.existingBankId, statement, fileName, fileUrl, rawTextContent, concurrencyCheck.reason);
        case 'CREATE_NEW':
        default:
            return await createNewBankAndStatement(statement, companyId, fileName, fileUrl, rawTextContent, concurrencyCheck.reason);
    }
}
/**
 * Merge transactions into an existing statement
 */
async function mergeTransactionsIntoExistingStatement(existingStatementId, newTransactions, newPeriodStart, newPeriodEnd, reason) {
    ...
    const expandedPeriodStart = new Date(Math.min(existingStatement.statementPeriodStart.getTime(), newPeriodStart.getTime()));
    const expandedPeriodEnd = new Date(Math.max(existingStatement.statementPeriodEnd.getTime(), newPeriodEnd.getTime()));
 ...
 */
async function createNewStatementInExistingBank(existingBankId, statement, fileName, fileUrl, rawTextContent, reason) {
 ...
 */
async function createNewBankAndStatement(statement, companyId, fileName, fileUrl, rawTextContent, reason) {
    const startingBalance = convertToDecimal(statement.starting_balance);
    const endingBalance = convertToDecimal(statement.ending_balance);
    const statementPeriodStart = new Date(statement.statement_period.start_date);
    const statementPeriodEnd = new Date(statement.statement_period.end_date);
...
//# sourceMappingURL=bankStatementConcurrencyService.js.map