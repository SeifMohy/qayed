...
): Promise<{ chunk: Uint8Array; pageRange: { start: number; end: number } }[]> {
  try {
...
async function processChunkWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        ...
        logger.info(
          `Retrying chunk ${chunkIndex + 1}/${totalChunks} (pages ${pageRange.start}-${pageRange.end}) for ${fileName}, attempt ${attempt + 1}/${maxRetries + 1} after ${retryDelay}ms delay`
        );
        await delay(retryDelay);
      } else {
        logger.info(
          `Processing chunk ${chunkIndex + 1}/${totalChunks} (pages ${pageRange.start}-${pageRange.end}) for ${fileName}`
        );
      }
      ...
      for await (const chunk of streamingResponse) {
        ...
        if (chunkText.trim()) {
          logger.info(
            `Chunk ${chunkIndex + 1} streaming: received ${chunkText.length} characters, total: ${accumulatedText.length}`
          );
        }
      }
      if (!accumulatedText || accumulatedText.trim() === "") {
        logger.warn(
          `GenAI returned empty text content for chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end}) of ${fileName}`
        );
        return "";
      } else {
        logger.info(
          `Successfully processed chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end}), extracted ${accumulatedText.length} characters`
        );
        const pageMarker =
      ...
      lastError = error;
      logger.error(
        `Error processing chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end}) of ${fileName}, attempt ${attempt + 1}:`,
        error.message
      );
      if (attempt === maxRetries || !isRetryableError(error)) {
        if (!isRetryableError(error)) {
          logger.info(
            `Error for chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end}) is not retryable, failing immediately`
          );
        } else {
          logger.info(
            `Max retries (${maxRetries}) reached for chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end})`
          );
        }
        break;
      }
  ...
  throw new Error(
    `Failed to process chunk ${chunkIndex + 1} (pages ${pageRange.start}-${pageRange.end}) after ${maxRetries + 1} attempts: ${lastError.message}`
  );
}
    ...
    try {
      ...
      for (let i = 0; i < pdfChunks.length; i++) {
        try {
          ...
          const text = await processChunkWithRetry(
            ai,
            pdfChunks[i].chunk,
            i,
            pdfChunks.length,
            fileName,
            pdfChunks[i].pageRange
          );
          ...
          logger.error(
            `Final error processing chunk ${i + 1} (pages ${pdfChunks[i].pageRange.start}-${pdfChunks[i].pageRange.end}) of ${fileName}:`,
            chunkError
          );
      ...
      const successfulChunks = chunkResults.filter(
        (text) => text.length > 0 && !text.includes("[Error processing chunk")
      ).length;
      ...
      logger.info(
        `Successfully processed ${fileName}: ${successfulChunks}/${pdfChunks.length} chunks successful, total length: ${combinedText.length}`
      );
      if (failedChunks > 0) {
