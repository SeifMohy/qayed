...
router.post('/structure', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const structuredData = await structureBankStatement(
        statementText,
        fileName,
        sendSSEMessage
      );
...
router.post('/parse-and-structure', uploadConfig.array('files'), async (req: Request, res: Response) => {
  try {
    const files = req.files as Express.Multer.File[];
    ...
    try {
      ...
      const parseSSEHandler = (data: SSEMessage) => {
        // Forward the message to client
        sendSSE(res, data);
        
        // Capture extracted text from file_complete messages
        if (data.type === 'file_complete' && data.success && (data as any).extractedText) {
          extractedTexts.push({
            fileName: data.fileName || 'unknown',
            text: (data as any).extractedText
          });
        }
      };
