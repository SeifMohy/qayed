...
async function saveDebugResponse(responseText, fileName, error) {
    try {
...
function performAutoValidation(statement) {
    try {
        ...
        const transactionsWithDates = transactions.filter((t) => t.transactionDate).length;
        const transactionsWithAmounts = transactions.filter((t) => (t.creditAmount && Number(t.creditAmount) > 0) ||
            (t.debitAmount && Number(t.debitAmount) > 0)).length;
 ...
 */
export async function structureBankStatement(statementText, fileName, fileUrl, supabaseUserId, sendSSE) {
    ...
    try {
        ...
        for (let i = 0; i < structuredData.account_statements.length; i++) {
            ...
            try {
                ...
                const result = await processBankStatementWithConcurrency(concurrencyStatement, supabaseUserId, fileName, fileUrl, statementText);
        ...
        const statementsWithCounts = await Promise.all(savedStatementIds.map(async (statementId) => {
