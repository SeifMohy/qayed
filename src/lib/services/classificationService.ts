...
/**
 * Automatically classify transactions for a specific bank (all statements)
 * @param bankId - The ID of the bank to classify transactions for
 * @returns Classification result summary
 */
export async function classifyBankTransactions(bankId: number): Promise<{
  success: boolean;
  classifiedCount: number;
  totalTransactions: number;
  bankStatementsProcessed: number;
  errors: string[];
}> {
  if (!API_KEY) {
    throw new Error('Gemini API key not configured');
  ...
  try {
    console.log(`Starting automatic classification for bank ${bankId}`);
    ...
    if (bankStatements.length === 0) {
      throw new Error('No bank statements found for this bank');
    }
    ...
    if (allTransactions.length === 0) {
      console.log(`No transactions found for bank ${bankId}`);
      return {
        success: true,
        classifiedCount: 0,
        totalTransactions: 0,
    ...
    console.log(`Classifying ${allTransactions.length} transactions across ${bankStatements.length} statements for bank ${bankId}`);
    ...
    console.error(`Error classifying bank ${bankId}:`, error);
    throw error;
  }
} 